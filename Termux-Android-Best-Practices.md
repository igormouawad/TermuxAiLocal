...
- Restart Termux, Termux:X11 and Termux:API before each run. The host reset helper (`ADB/adb_reset_termux_stack.sh`) automates this, verifies that `Termux:API` stayed alive, and optionally focuses either Termux or Termux:X11 while keeping the split pair restored.
- After each non-reinstall restart of the GUI pair, restore `Termux` and `Termux:X11` side by side in split-screen, with `Termux` on the left and `Termux:X11` on the right.
